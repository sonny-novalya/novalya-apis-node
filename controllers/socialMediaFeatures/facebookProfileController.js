...
self.getDashboardSocialAccountData = async (req, res) => {
    ...
    try {
        ...
        const statistics = await Statistic.findAll({
            ...
            attributes: [
                [fn("SUM", literal("CASE WHEN type IN ('fb_prospection', 'birthday', 'requests', 'fb_crm', 'ig_crm') THEN message_count ELSE 0 END")), "fbMessageLimit"],
        ...
        const fbCount = await tagsTable.count({ where: { user_id: user_id } });
        const igCount = await instaGramTag.count({ where: { user_id: user_id } });

        console.log('tagFBAiLimit', fbCount);
        console.log('tagIGAiLimit', igCount);
        const final_ai_limit = fbCount + igCount;
